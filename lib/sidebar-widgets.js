...
(() => {
  ...
  function _localLayoutKey(role, user) {
    const me = window.me?.();
    const ident = user?.id || me?.user_id || me?.handle || "local";
  ...
  function _writeLocalLayout(role, layout, user) {
    try { localStorage.setItem(_localLayoutKey(role, user), JSON.stringify(layout || [])); } catch {}
  }
  ...
  window.saveSidebarLayout = async function (layout, activeRole) {
    ...
    window.dispatchEvent(new CustomEvent("sidebar:updated", { detail: { layout, role } }));
    ...
    try {
      ...
      _writeLocalLayout(role, layout, user);
  ...
  window.loadSidebarLayout = async function (activeRole) {
    const role = _layoutRole(activeRole);
    const sb = window.getSupabase && window.getSupabase();
    if (!sb) return _readLocalLayout(role) || _roleDefault(role);
    try {
      const { data: { user } } = await sb.auth.getUser();
      if (!user) return _readLocalLayout(role) || _roleDefault(role);
      ...
      _writeLocalLayout(role, data.layout, user);
      return data.layout;
    } catch (e) {
      return _readLocalLayout(role) || _roleDefault(role);
    }
  };
