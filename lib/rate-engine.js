...
(function () {
  // ── Carrier underwriting guide loader ──────────────────────────────────
  // Pulls /lib/carrier-underwriting.json (real producer-guide data) into
  // window.CARRIER_UW_GUIDES so the rate engine can cite the actual rule
  // that drove a decline or UW class assignment.
  //
  // JSON keys (uhc_aarp / mutual_omaha / aetna_src / final_expense / ma /
  // giwl_final_expense / myga) get normalized to the short carrier ids the
  // engine + niches table use (uhc / moo / aetna / fe / mapd / fe / annuity).
  ...
  let UW_GUIDES = {};
  function normalizeGuides(json) {
    const out = {};
    const carriers = json?.carriers || {};
    for (const [rawId, rec] of Object.entries(carriers)) {
      const id = CARRIER_KEY_MAP[rawId] || rawId;
      const products = {};
      for (const [rawProd, body] of Object.entries(rec.products || {})) {
        const prod = PRODUCT_KEY_MAP[rawProd] || rawProd;
        products[prod] = { ...body, _carrierName: rec.name };
      }
      out[id] = { id, name: rec.name, products };
    }
    return out;
  }
  function loadGuides() {
    try {
      fetch("/lib/carrier-underwriting.json", { cache: "force-cache" })
        .then(r => r.ok ? r.json() : null)
        .then(json => {
          if (!json) return;
          UW_GUIDES = normalizeGuides(json);
          window.CARRIER_UW_GUIDES = UW_GUIDES;
          window.dispatchEvent(new CustomEvent("carrier-uw:loaded"));
        })
        .catch(() => {});
    } catch (e) { console.warn("[rate-engine.loadGuides]", e); }
  }
  loadGuides();

  // ── DB hydration (overrides JSON when carriers+rules exist in Supabase) ─
  // Rebuilds UW_GUIDES from the product_underwriting_rules table so admin
  // edits via the Carriers admin tab take effect without a code redeploy.
  // Falls back silently to the JSON guides if the DB is empty or unreachable.
  function rulesToGuide(carrierName, productRows, ruleRowsByProduct) {
    ...
    for (const p of productRows) {
      ...
      const body = { _carrierName: carrierName };
      for (const r of rules) {
        ...
        body._sources = body._sources || [];
        if (r.source_url) body._sources.push({ url: r.source_url, excerpt: r.source_quote, rule_type: r.rule_type });
      }
      // Only set when DB actually has an opinion — otherwise the per-product
      // spread merge below would blow away JSON narrative values with undefined.
      ...
      if (p.features?.confidence != null)        body.confidence        = p.features.confidence;
      products[prod] = body;
    }
    return products;
  }
  ...
  async function hydrateFromSupabase() {
    try {
      ...
      if (carriers.length === 0 || products.length === 0) return; // DB empty → JSON wins
      ...
      // Merge per-product so DB rule fields (tobacco_rateup_pct, max_bmi,
      // auto_decline_conditions, …) override JSON, while JSON narrative
      // fields (sweet_spot, sources, discounts, tobacco_notes, build_notes,
      // uw_classes_notes, confidence) survive — recommendReasons() needs
      // them for the "Best pick · per official underwriting" citations.
      for (const c of carriers) {
        const shortId = CARRIER_KEY_MAP[c.id] || c.id;
        const prodRows = productsByCarrier.get(c.id) || [];
        const dbProducts = rulesToGuide(c.name, prodRows, rulesByProduct);
        const existing = UW_GUIDES[shortId] || { id: shortId, name: c.name, products: {} };
        const mergedProducts = { ...(existing.products || {}) };
        for (const [prodKey, dbBody] of Object.entries(dbProducts)) {
          const jsonBody = (existing.products || {})[prodKey] || {};
          mergedProducts[prodKey] = {
            ...jsonBody,
            ...dbBody,
            _sources: [...(jsonBody._sources || []), ...(dbBody._sources || [])],
          };
        }
        UW_GUIDES[shortId] = { id: shortId, name: c.name, products: mergedProducts };
      }
      window.CARRIER_UW_GUIDES = UW_GUIDES;
      window.dispatchEvent(new CustomEvent("carrier-uw:loaded", { detail: { source: "db" } }));
    } catch (e) {
      console.warn("[rate-engine] DB hydrate failed, keeping JSON guides:", e);
    }
  }
  ...
  function determineUwClass(carrier, profile, product) {
    ...
    // Hard declines first — cross-check against the real producer-guide
    // decline lists from carrier-underwriting.json. Each rule cites its source.
    const rxKill = rxAutoDecline(profile.prescriptions || []);
    if (rxKill) return { class: "DECLINE", reason: `Auto-decline · ${rxKill}` };

    // Guide-driven hard declines (the carrier-specific lists in the JSON)
  ...
  function recommendReasons(carrier, product, profile, rate) {
    ...
    if (Array.isArray(guide?.sources) && guide.sources.length) {
      for (const s of guide.sources.slice(0, 2)) {
        const path = (s.url || "").match(/[^/]+\.pdf$/i)?.[0] || s.url || "";
        if (path) src.push(path);
      }
    }
    return { reasons: out, sources: src, confidence: guide?.confidence || null };
  }
