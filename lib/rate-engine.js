...
(function () {
  ...
  function rulesToGuide(carrierName, productRows, ruleRowsByProduct) {
    ...
    for (const p of productRows) {
      ...
      body.underwriting_type = p.features?.underwriting_type;
      body.uw_classes = p.features?.uw_classes;
      body.confidence = p.features?.confidence;
      products[prod] = body;
    }
    return products;
  }
  ...
  async function hydrateFromSupabase() {
    try {
      ...
      // Merge: DB carriers override JSON, but JSON-only carriers are preserved.
      UW_GUIDES = { ...UW_GUIDES, ...fromDb };
      window.CARRIER_UW_GUIDES = UW_GUIDES;
