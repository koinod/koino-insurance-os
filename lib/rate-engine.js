...
(function () {
  ...
  const CARRIER_KEY_MAP = {
    ...
    ethos:            "ethos",
    americanamicable: "americanamicable",
  ...
  const PRODUCT_KEY_MAP = {
    ...
    med_supp:                    "medsupp",
    ma:                          "mapd",
    medicare_advantage:          "mapd",
    final_expense:               "fe",
    giwl_final_expense:          "fe",
    whole_life_fe:               "fe",
    whole_life_fe_immediate:     "fe",
    whole_life_fe_simplified:    "fe",
    whole_life_fe_graded:       "fe",
    whole_life:                  "fe",
    term:                        "term",
    life:                        "term",
    iul:                         "iul",
  ...
  const CARRIER_DELTA = {
    ...
    americo:          { fe: 0.96, term: 0.94, iul: 0.98, annuity: 1.00 },     // Eagle Premier FE, CPT Term, Instant Decision IUL, MYGA
  ...
  function calculateAnnuityYield(carrier, profile) {
    const annuityCarriers = ["fg", "americo", "corebridge", "aig", "transamerica", "moo"];
    if (!annuityCarriers.includes(carrier?.id)) return null;
    const premium = profile.premium || 50000;
    const term = profile.term || 5;  // years
    const apy = carrier.id === "americo" ? 5.35 : 5.25;  // Americo Platinum Assure MYGA vs baseline
