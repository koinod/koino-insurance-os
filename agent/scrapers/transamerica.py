"""Transamerica producer-portal scraper (FE + Term + IUL)."""
import re

REQUIRES_LOGIN = True
CARRIER_NAME = "Transamerica"
LOGIN_URL = "https://www.transamerica.com/agent"
LOGGED_IN_INDICATOR = "selector:a:has-text('Sign out'), a:has-text('Logout'), [data-testid*='dashboard']"
QUOTE_URL = "https://www.transamerica.com/agent"
...
def quote(profile: dict, page, creds=None) -> dict:
    try:
        ...
        if "login" in (page.url or "").lower():
            return {"decline": True, "reason": "session expired — re-capture Transamerica login"}
        ...
        return {"decline": True, "reason": "Transamerica scraper needs selector mapping — run `inspect transamerica` after capturing session", "raw": body[:600]}
