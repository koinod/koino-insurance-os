...
(function () {
  ...
  function PageCrmWorkspace({ role = "manager" }) {
    ...
    return <div className="page-pad crm-workspace" data-crm-view={view}>
      ...
      <div className="crm-view-head"><div><h2>{title}</h2><span>{view === "pipeline" ? `${filtered.length} active records` : view === "clients" ? `${filtered.filter(r => r.client).length} clients` : view === "money" ? `${moneyRows.deposits.length + moneyRows.expenses.length} ledger entries` : `${appts.length} carrier appointments`}</span></div>{view === "carriers" && <button className="btn" onClick={() => window.gotoPage?.("carrier-appointments")}>Edit appointments</button>}</div>
      ...
      {view === "carriers" && <CarriersView carriers={carriers} appts={appts} policies={policies} leads={allLeads} clawbacks={clawbacks} allocations={moneyRows.allocations} />}
  ...
  function CarriersView({ carriers, appts, policies, leads, clawbacks, allocations }) {
    const [active, setActive] = useState(null);
    const carrierById = Object.fromEntries(carriers.map(c => [c.id, c]));
    const leadById = Object.fromEntries(leads.map(l => [l.id, l]));
    const rows = appts.map(appt => {
      const carrier = carrierById[appt.carrierId] || { id: appt.carrierId, name: appt.carrierName || "Unknown carrier" };
      const carrierPolicies = policies.filter(p => p.carrierId === appt.carrierId);
      const carrierDebts = clawbacks.filter(d => carrierPolicies.some(p => p.id === d.policyId) && ["recorded", "disputing"].includes(d.status || "recorded"));
      const paidCents = allocations.filter(a => carrierPolicies.some(p => p.id === a.policy_id)).reduce((n, a) => n + (Number(a.amount_cents) || 0), 0);
      return { appt, carrier, policies: carrierPolicies, debts: carrierDebts, paid: paidCents / 100, ap: carrierPolicies.reduce((n, p) => n + (p.ap || 0), 0), leadById };
    });
    return <>
      <div className="crm-carrier-grid">{rows.map(row => <button key={row.appt.id || row.carrier.id} className="crm-carrier-card" onClick={() => setActive(row)}><div className="crm-carrier-card-head"><div><strong>{row.carrier.name}</strong><small>{row.carrier.category || "Life & health"} · {row.appt.appointedStates?.length || 0} states</small></div><Badge>{row.appt.status || "pending"}</Badge></div><div className="crm-carrier-card-stats"><div><strong>{row.policies.length}</strong><span>Deals</span></div><div><strong>{money(row.ap)}</strong><span>Written AP</span></div><div><strong>{money(row.paid)}</strong><span>Paid</span></div><div className={row.debts.length ? "crm-carrier-debt" : ""}><strong>{money(row.debts.reduce((n, d) => n + (d.amount || 0), 0))}</strong><span>Rollup debt</span></div></div><div className="crm-carrier-card-foot"><span>{row.appt.productLines?.join(", ") || "Product requirements not entered"}</span><Icons.ArrowUpRight size={13}/></div></button>)}</div>{!rows.length && <div className="crm-empty">No agency carrier appointments yet. Add them from Edit appointments.</div>}{active && <CarrierDetailDrawer row={active} onClose={() => setActive(null)} />}</>;
  }

  function CarrierDetailDrawer({ row, onClose }) {
    const { appt, carrier, policies, debts, paid, ap, leadById } = row;
    return <div className="crm-drawer-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}><aside className="crm-drawer"><div className="crm-drawer-head"><div><div className="crm-eyebrow">Carrier appointment</div><h2>{carrier.name}</h2><div className="crm-muted">{carrier.category || "Life & health"} · {appt.status || "pending"}</div></div><button className="crm-icon-btn" onClick={onClose} aria-label="Close">×</button></div><div className="crm-drawer-actions"><button className="btn btn-primary" onClick={() => window.gotoPage?.("carrier-appointments")}>Edit appointment</button>{appt.portalUrl && <a className="btn" href={appt.portalUrl} target="_blank" rel="noreferrer">Open portal</a>}</div><div className="crm-detail-section"><div className="crm-detail-label">Rollup</div><div className="crm-detail-stats"><div><span>Deals written</span><strong>{policies.length}</strong></div><div><span>Written AP</span><strong>{money(ap)}</strong></div><div><span>Paid / deposited</span><strong>{money(paid)}</strong></div><div><span>Rollup debt</span><strong className={debts.length ? "crm-bad-text" : "crm-good-text"}>{money(debts.reduce((n, d) => n + (d.amount || 0), 0))}</strong></div></div></div><div className="crm-detail-section"><div className="crm-detail-label">Appointment information</div><div className="crm-detail-row"><span>Status</span><Badge>{appt.status || "pending"}</Badge></div><div className="crm-detail-row"><span>NPN</span><strong>{appt.npn || "Not entered"}</strong></div><div className="crm-detail-row"><span>States</span><strong>{appt.appointedStates?.join(", ") || "Not entered"}</strong></div><div className="crm-detail-row"><span>Product lines</span><strong>{appt.productLines?.join(", ") || "Not entered"}</strong></div><div className="crm-detail-row"><span>Bridge under</span><strong>{appt.bridgeUnderName || appt.bridgeUnderNpn || "Direct / none"}</strong></div>{appt.notes && <div className="crm-form-note" style={{ marginTop: 12 }}>{appt.notes}</div>}</div><div className="crm-detail-section"><div className="crm-detail-label">Deals written</div>{policies.length ? policies.map(p => <div className="crm-policy-row" key={p.id}><div><strong>{leadById[p.leadId]?.lead || p.product || "Policy"}</strong><div className="crm-muted">{p.product || "Product not entered"} · {p.policyNumber || "No policy number"}</div></div><div className="crm-policy-right"><Badge>{p.status || "pending"}</Badge><strong>{money(p.ap)}</strong></div></div>) : <div className="crm-empty-inline">No deals written with this carrier yet.</div>}</div></aside></div>;
  }
